...
export const sendSelectionNotification = async (candidateName, candidateEmail, statusRecord) => {
  ...
  const appId = statusRecord.application_id || statusRecord.tracking_id;
  ...
  const bodyHtml = wrapHtmlEmail('Internship Selection Confirmed', `
    ...
    <div class="highlight-box">
      <table style="width:100%;">
        <tr><td style="color:#A0A0A0;width:40%;">Application ID</td><td style="color:#C8A96A;font-weight:bold;">${appId}</td></tr>
  ...
  const transporter = createTransporter();
  if (!transporter) {
    console.log('[SIMULATION] Selection email to ' + candidateEmail + ' — Application ID: ' + appId);
    return true;
  }
  await transporter.sendMail({
    from: `"Manchester Technologies" <${process.env.SMTP_USER}>`,
...
/**
 * Sends signature confirmation with Application ID after signing.
 */
export const sendSignatureConfirmation = async (candidateName, candidateEmail, applicationId, certId, signedAt) => {
  const portalUrl = process.env.PORTAL_URL || 'http://localhost:5173';
  const appId = applicationId || certId;
  const subject = `Agreement Signed Successfully — Application ID: ${appId}`;
  ...
  const bodyHtml = wrapHtmlEmail('Digital Agreement Signed', `
    ...
    <div class="highlight-box">
      <table style="width:100%;">
        <tr><td style="color:#A0A0A0;width:40%;">Application ID</td><td style="color:#C8A96A;font-weight:bold;font-size:15px;">${appId}</td></tr>
    ...
    <p style="color:#A0A0A0;font-size:12px;">Keep Application ID <strong style="color:#C8A96A;">${appId}</strong> safe as proof of acceptance.</p>
    <p>Best Regards,<br/><strong>Manchester Technologies</strong></p>
  `);
  const transporter = createTransporter();
  if (!transporter) {
    console.log('[SIMULATION] Signature confirmation to ' + candidateEmail + ' — Application ID: ' + appId);
    return true;
  }
  await transporter.sendMail({
    from: `"Manchester Technologies" <${process.env.SMTP_USER}>`,
